...
description: >
  ...
  harbor sandbox, HarborSandbox, headless CI, non-interactive dcode,
...
| `-n`, `-y`, piped stdin, `--sandbox`, CI | Headless / CI | `references/headless-ci.md` |
...
| `references/headless-ci.md` | `-n`/`-y` modes, piped stdin, MLflow tracing, non-interactive conventions | CI or scripted dcode usage |
...
| MLflow tracing | Set `MLFLOW_TRACKING_URI` to route traces to your MLflow server |
...
# Verify MLflow tracing is active
MLFLOW_TRACKING_URI=http://localhost:5000 dcode -n "echo hello"
# Traces appear in the MLflow experiment at the configured tracking URI
```
...
| [references/headless-ci.md](references/headless-ci.md) | -n/-y modes, piped stdin, MLflow tracing, non-interactive tips | CI/scripted dcode or automation |
