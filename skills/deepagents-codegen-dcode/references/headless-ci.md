...
dcode emits traces for agent LLM calls and shell command (execute) calls via `mlflow.langchain.autolog()`. Set `MLFLOW_TRACKING_URI` to route traces to your MLflow server:

```bash
MLFLOW_TRACKING_URI=http://localhost:5000 dcode -n "Run tests"
```

This lets you:
- Monitor agent reasoning cost separately from execution cost using MLflow experiments
- Audit shell commands independently from LLM calls
- Compare trace timelines across dcode runs using MLflow's experiment tracking UI
