...
Per-criterion grader feedback appears in MLflow traces as grader subagent spans. Enable MLflow autolog to inspect which criteria passed/failed on each iteration:

```bash
MLFLOW_TRACKING_URI=http://localhost:5000 uv run python your_agent.py
```
