...
`RubricMiddleware` grader runs appear as child spans in MLflow traces. Connect with the `observability` skill to:
...
1. **Check MLflow traces** — inspect grader subagent's per-criterion output for each iteration.
