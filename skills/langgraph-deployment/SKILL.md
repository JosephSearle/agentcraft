...
argument-hint: "[dev|docker|scale]"
description: >
...
If invoked with `$ARGUMENTS`, focus exclusively on that mode (`dev` for local development, `docker` for containerised setup, `scale` for horizontal scaling) and skip other sections.
